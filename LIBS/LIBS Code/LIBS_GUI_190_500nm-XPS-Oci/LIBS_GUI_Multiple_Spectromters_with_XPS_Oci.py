...
class ocilloscopeThread(threading.Thread):
    def __init__(self, data_source, trig_source, v_div, t_div):
        super(ocilloscopeThread,self).__init__()
        # self.start = start
        global data_oci
        data_oci=self.ocilloscope(data_source, trig_source, v_div, t_div)
    def ocilloscope(self, data_source, trig_source, v_div, t_div):
...
#-------------------------------------------------------------------------------------   
    ...
    def MySpectrometers(self):
        ...
        ocilloscopeTrigThread = threading.Thread(target=ocilloscopeThread, args=(self.data_source,self.trig_source,self.v_div, self.t_div, ))
